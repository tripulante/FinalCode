...
(let*
    ((sectionlist  (loop for i from 11 downto 1 collect i))
     (sc (make-instance 'sorting-class))
     (maplist (let ((results nil))
		(print sectionlist)
		(insertion-sort sc (copy-list sectionlist))
		(setf results (subsorts sc))
		(clear-stored-sort sc)
		(merge-sort sc sectionlist)
		(setf results (append (subsorts sc) results))
		(clear-stored-sort sc)
		(quick-sort sc (copy-list sectionlist) 0 (1- (length sectionlist)))
		(setf results (append (subsorts sc) results))
		(bubble-sort sc (copy-list sectionlist))
		(setf results (append (subsorts sc) results))
		(print results)
		(flatten results)))
     (template (make-slippery-chicken
		'+cycle+
		:title "Sorting Template"
		:composer "John Palma"
